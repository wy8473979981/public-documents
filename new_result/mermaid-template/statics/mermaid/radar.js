...
// 尝试获取 label-map-data 中的配置
const labelMapDataElement = document.getElementById('label-map-data');
let mermaidConfig = {};
if (labelMapDataElement && labelMapDataElement.textContent.trim()) {
    try {
        mermaidConfig = JSON.parse(labelMapDataElement.textContent);
    } catch (e) {
        console.error('Mermaid 配置解析失败:', e);
    }
}
