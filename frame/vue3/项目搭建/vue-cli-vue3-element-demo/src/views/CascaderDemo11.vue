...
// 生成节点顺序映射表（path字符串 → 顺序号）
function buildOrderMap(options, orderMap = {}, parentPath = []) {
  // 遍历传入的选项数组
  options.forEach((node) => {
    // 构建当前节点的路径，将父路径和当前节点的id拼接起来
    const path = [...parentPath, node.id];
    // 将当前路径作为键，顺序号作为值存入orderMap中
    // 顺序号是通过获取orderMap的键的数量来确定的
    orderMap[path.join(',')] = Object.keys(orderMap).length; // 顺序号
    // 如果当前节点有子节点
    if (node.children?.length) {
      // 递归调用buildOrderMap函数，传入当前节点的子节点、orderMap和当前路径
      buildOrderMap(node.children, orderMap, path);
    }
  });
  // 返回构建好的orderMap
  return orderMap;
...
function sortSelectionsByOptions(selections, options) {
  ...
  return [...selections].sort(
    // 自定义排序函数
    (a, b) => {
      // 获取 a 的顺序值，若不存在则默认为 99999
      const orderA = orderMap[a.join(',')] ?? 99999;
      // 获取 b 的顺序值，若不存在则默认为 99999
      const orderB = orderMap[b.join(',')] ?? 99999;
      // 返回两个顺序值的差值，用于排序
...
const mergedSelections = computed(() => {
  const merged = sortSelectionsByOptions(
...
function getNamePath(idPath) {
  ...
  for (const id of idPath) {
    ...
    // 更新当前节点集合为匹配节点的子节点集合，如果没有子节点，则为一个空数组
    nodes = node.children || [];
  }
...
function removeTag(path) {
  ...
  // 更新 summaryMap（移除父节点本身 & 子孙节点）
  enabledPaths.forEach((p) => {
    const id = p[p.length - 1];
    const childNode = findNode(newLocationOptions.value, id);
    if (childNode) {
      updateSummaryMap(childNode, 'deselect');
    }
  });
...
function handleChange(paths) {
  ...
  console.log('当前 prev:', JSON.stringify(prev));
